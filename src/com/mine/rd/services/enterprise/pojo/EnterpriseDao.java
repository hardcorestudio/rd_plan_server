...
//				System.out.println("tablename=====>"+tablename);
	...
	public boolean deleteNoApproveAdmin(String epId, String tablename){
		...
		if(persons.size() > 0){
			for(Record person : persons){
				if(Db.update("delete from "+tablename+" where USER_ID = ?", person.get("USER_ID")) <= 0){
					return false;
				}
				if(Db.update("delete from WOBO_USER where USER_ID = ?", person.get("USER_ID")) <= 0){
					return false;
				}
				if(Db.update("delete from WOBO_USER_INFO where USER_ID = ?", person.get("USER_ID")) <= 0){
					return false;
				}
			}
		}
