...
public class LoginDao extends BaseDao {
	...
	public Map<String, Object> loginSysadmin(String username){
		String sql = "select a.USER_ID,a.NAME,c.PORTRAIT,a.PWD from WOBO_USER a, WOBO_USER_INFO c where a.STATUS = '1'  and a.USER_ID = c.USER_ID  and a.NAME=?  "+super.checkLowerAndUpper();
		Record record =  Db.findFirst(sql, username);
		Map<String, Object> map = new HashMap<>();
		if(record != null){
			map.put("userId", record.get("USER_ID"));
...
//				userInfo.set("PWD", pwd);
	 ...
	 * 
	 */
	public Map<String, Object> verifyLoginInfo(String PWD, String REGISTERCODE){
		Record record = new Record();
		String sql = "";
		sql = "select * from ENTERPRISE a, USER_PWD b where a.EP_ID = b.userId and a.IF_PRODUCE = '1' and ep_id=? "+super.checkLowerAndUpper();
		record = Db.findFirst(sql, REGISTERCODE);
		Map<String, Object> map = new HashMap<String, Object>();
		if(record != null){
