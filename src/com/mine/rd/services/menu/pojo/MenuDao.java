...
public class MenuDao extends BaseDao {
	...
	public List<Map<String, Object>> queryMenu() {
		String sql = "select * from Z_WOBO_FUNC where STATUS in ('0','1') order by SORT asc";
	...
	public String querySonMenu(String menuId){
		List<Record> list = Db.find("select * from Z_WOBO_FUNC where FUNC_PARENT = ? and STATUS in ('0','1')", menuId);
		if(list.size()>0){
			return "folder";
		}else{
			return "doc";
	...
	public Map<String, Object> queryMenuInfo(String menuId){
		Record record = Db.findFirst("select * from Z_WOBO_FUNC where FUNC_ID = ? and STATUS in ('0','1')", menuId);
	...
	public boolean updateMenu(String menuId, String status, String menuName){
		String sql1 = "update Z_WOBO_FUNC set STATUS = ?, NAME=? where FUNC_ID = ?";
		String sql2 = "update Z_WOBO_FUNC_ATTR set STATUS = ? where FUNC_ID = ?";
		...
		for(Record func : list){
			flag = Db.update("update Z_WOBO_FUNC set STATUS = ? where FUNC_ID = ?", status, func.get("FUNC_ID")) > 0;
			if(!flag){
				return false;
			}
