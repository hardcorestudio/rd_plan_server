...
//		Db.update("delete from Z_WOBO_HANDLESELF_LIST where tp_id = ? " ,tpId);
	...
	public String getLastTBSum(String epId,String en_cz_id,String bigId,String smallId,String unit,String D_NAME){
		...
		sb.append(" and b.UNIT = ? and b.BIG_CATEGORY_ID = ? and SAMLL_CATEGORY_ID = ? and EN_ID_CS = ? and EN_ID_CZ = ? and b.D_NAME= ? ");
		sb.append(" and DATEDIFF(yyyy,a.ACTIONDATE,DATEADD(year,-1,GETDATE())) = 0 ");
		Record record = Db.findFirst(sb.toString(),unit,bigId,smallId,epId,en_cz_id,D_NAME);
		if(record != null && record.get("last_unit_num") != null){
			last_unit_num = record.get("last_unit_num") + "";
		}
		return last_unit_num;
	}
	public List<Map<String,Object>> initPt(String tpId){
...
//		return map;
//	}
	public List<Map<String,Object>> initPtList(String tpId,String no){
		List<Record> records = Db.find("select * from Z_WOBO_TRANSFER_PLAN_LIST_PT where tp_id=? and no = ? ",tpId,no);
		List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
		for(int i = 0; i<records.size() ; i++){
			list.add(records.get(i).getColumns());
		}
	...
	public boolean savePt(String tpId, String count,String EN_ID_CS, String EN_NAME_CS,String ysdwmc,
...
//		Db.update("delete from Z_WOBO_TRANSFER_PLAN_PT where tp_id = ? " ,tpId);
	...
	public boolean savePtList(String tpId,String no,List<Map<String,Object>> list){
