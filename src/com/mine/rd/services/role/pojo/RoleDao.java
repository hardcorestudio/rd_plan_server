...
public class RoleDao extends BaseDao {
	...
	public Map<String, Object> queryRoleList(){
		List<Record> record = Db.find("select * from Z_WOBO_ROLE where STATUS != '2'");
	...
	public Map<String, Object> queryRoleInfo(String roleId){
		Record record = Db.findFirst("select * from Z_WOBO_ROLE where STATUS != '2' and ROLE_ID = ?", roleId);
		Map<String, Object> map = new HashMap<>();
		if(record != null){
	...
	public boolean checkRoleName(String roleId, String roleName){
		String sql = "";
		if("".equals(roleId)){
			sql = "select * from Z_WOBO_ROLE where NAME = ? and STATUS != '2'";
		}else{
			sql = "select * from Z_WOBO_ROLE where NAME = ? and STATUS != '2' and ROLE_ID != '"+roleId+"'";
		}
	...
	public String addOrUpdateRole(String roleId, String roleName, String status){
		if("".equals(roleId)){
			...
			String id = super.getSeqId("Z_WOBO_ROLE");
			record.set("ROLE_ID", id);
			record.set("NAME", roleName);
			record.set("STATUS", "0");
			record.set("sysdate", super.getSysdate());
			if(Db.save("Z_WOBO_ROLE", "ROLE_ID", record)){
				return id;
			}else{
				return "";
			}
		}else{
			boolean flag = Db.update("update Z_WOBO_ROLE set NAME = ?, STATUS = ? where ROLE_ID = ?", roleName, status, roleId) > 0;
			if(flag){
				return roleId;
			}else{
				return "";
	...
	public boolean delRole(String roleId){
		return Db.update("update Z_WOBO_ROLE set STATUS = '2' where ROLE_ID = ?", roleId) > 0;
	}
	...
	public List<Map<String, Object>> queryRoleMenu(String roleId){
		List<Record> record = Db.find("select a.*,b.ROLE_ID from Z_WOBO_FUNC a left join Z_WOBO_ROLEFUNC b on a.FUNC_ID = b.FUNC_ID and b.ROLE_ID = ? and b.STATUS = '1' where a.STATUS = '1'", roleId);
		List<Map<String, Object>> list = new ArrayList<>();
		if(record.size() > 0){
	...
	public boolean manageRoleMenu(String roleId, String menuIdList){
		Record menu = Db.findFirst("select * from Z_WOBO_ROLEFUNC where ROLE_ID = ?", roleId);
		if(menu != null){
			boolean flag = Db.update("delete from Z_WOBO_ROLEFUNC where ROLE_ID = ?", roleId)>0;
			if(!flag){
				return false;
			}
		}
		if(!"".equals(menuIdList)){
			...
			for(String menuId : menuIds){
				...
				record.set("STATUS", "1");
				if(!Db.save("Z_WOBO_ROLEFUNC", record)){
					return false;
				}
				Record func = Db.findFirst("select * from Z_WOBO_FUNC where FUNC_ID = ? and STATUS = '1'", menuId);
				if(func != null && func.get("FUNC_PARENT") != null && !"".equals(func.get("FUNC_PARENT"))){
					Record funcRole = Db.findFirst("select * from Z_WOBO_ROLEFUNC where FUNC_ID = ? and ROLE_ID = ? and STATUS = '1'", func.get("FUNC_PARENT"),roleId);
					if(funcRole == null){
						Record record1 = new Record();
						record1.set("ROLE_ID", roleId);
						record1.set("FUNC_ID", func.get("FUNC_PARENT"));
						record1.set("STATUS", "1");
						if(!Db.save("Z_WOBO_ROLEFUNC", record1)){
							return false;
						}
					}
				}
	...
	public Map<String, Object> queryRoleUser(String roleId, int pn, int ps, Object searchContent){
		...
		if("CSDWGLY".equals(roleId)){
			...
			sql = "from (select A.*,B.ROLE_ID from (select cast(c.OPERATORID as varchar(20)) as USER_ID, c.USERID as NAME, b.ORGCODE as EP_ID,b.ORGNAME as EP_NAME from OM_EMPLOYEE a, OM_ORGANIZATION b, AC_OPERATOR c, AC_OPERATORROLE d where a.ORGID = b.ORGID and a.OPERATORID = c.OPERATORID and c.STATUS = 'running' and c.OPERATORID = d.OPERATORID and d.ROLEID='SJSPROLE') as A left join Z_WOBO_USERROLE B on A.USER_ID = B.USER_ID and B.ROLE_ID = ?) as AA";
		}else if("QJGLY".equals(roleId)){//区级管理员
			sql = "from (select A.*,B.ROLE_ID from (select cast(c.OPERATORID as varchar(20)) as USER_ID, c.USERID as NAME, b.ORGCODE as EP_ID,b.ORGNAME as EP_NAME from OM_EMPLOYEE a, OM_ORGANIZATION b, AC_OPERATOR c, AC_OPERATORROLE d where a.ORGID = b.ORGID and a.OPERATORID = c.OPERATORID  and c.STATUS = 'running' and c.OPERATORID = d.OPERATORID and d.ROLEID='QJSPROLE') as A left join Z_WOBO_USERROLE B on A.USER_ID = B.USER_ID  and B.ROLE_ID = ?) as AA";
		}else if("ADMIN".equals(roleId)){//系统管理员
			sql = "from (select A.*,B.ROLE_ID from (select a.USER_ID,a.NAME ,'sysadmin' as EP_ID,'sysadmin' as EP_NAME from Z_WOBO_USER a where a.STATUS= '1'  ) as A left join Z_WOBO_USERROLE B on A.USER_ID = B.USER_ID and B.ROLE_ID = ?) as AA";
		...
		}else if("CSEP".equals(roleId)){
			sql = "from (select A.*,B.ROLE_ID from (select cast(c.USERID as varchar(20)) as USER_ID, a.EP_NAME as NAME, a.EP_ID,a.EP_NAME from ENTERPRISE a, AC_OPERATOR c where a.EP_ID = c.USERID and c.STATUS = 'running' and a.STATUS = '2' and a.IF_PRODUCE = '1' ) as A left join Z_WOBO_USERROLE B on A.USER_ID = B.USER_ID and B.ROLE_ID = ?) as AA";
		}else{
	...
	public boolean manageRoleUser(String roleId, String userIdList, String allUserId){
		if(!"".equals(allUserId)){
			...
			for(String userId : userIds){
				Record user = Db.findFirst("select * from Z_WOBO_USERROLE where ROLE_ID = ? and USER_ID = ? and STATUS = '1'", roleId,userId);
				if(user != null){
					boolean flag = Db.update("delete from Z_WOBO_USERROLE where ROLE_ID = ? and USER_ID = ?", roleId,userId)>0;
					if(!flag){
						return false;
					}
				}
		...
		if(!"".equals(userIdList)){
			...
			for(String userId : userIds){
				...
				record.set("STATUS", "1");
				if(!Db.save("Z_WOBO_USERROLE", record)){
					return false;
				}
			}
		}
